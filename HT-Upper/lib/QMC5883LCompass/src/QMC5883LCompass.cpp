...
/**
	获取非水平状态下的航向角
**/
int QMC5883LCompass::getAzimuthABS(float Pit, float Rol){

	const float YAW_LOW_PASS_FILTER_ALPHA = 0.0001; // 滤波系数，根据需要调整

	// 将角度转换为弧度
    float pitchRad = Pit * PI / 180.0;
    float rollRad = -Rol * PI / 180.0;
    ...
    // 补偿磁力计数据
    float magXCompensated = magX * cos(pitchRad) + magZ * sin(pitchRad);
    float magYCompensated = magX * sin(rollRad) * sin(pitchRad) + magY * cos(rollRad) - magZ * sin(rollRad) * cos(pitchRad);

    // 计算航向角
    float compensatedHeading = atan2(magYCompensated, magXCompensated) * 180.0 / PI;
	
	// 考虑磁偏角
    compensatedHeading += _magneticDeclinationDegrees;  

	// 应用低通滤波器
    filteredYaw = YAW_LOW_PASS_FILTER_ALPHA * filteredYaw + (1 - YAW_LOW_PASS_FILTER_ALPHA) * compensatedHeading;

	return (int)filteredYaw % 360;
}
